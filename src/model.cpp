...
int model::load_model(string model_name) {
  ...
  // GET MODEL'S PREVIOUSLY ASSIGNED ALPHA AND BETA VALUES
  filename = dir + model_name + others_suffix;
  ifstream file(filename.c_str());
  string w, values;

  if (!file) {
    printf("Cannot open file %s to load alpha and beta values!\n", filename.c_str());
    return 1;
  }

  alpha = new double[K];

  //first line: alpha values

  //skipping the 'alpha=' part on string
  getline(file, line, '=');
  //getting all alpha values
  getline(file, values, '\n');

  istringstream iss(values);

  // splitting string and setting alpha values
  for(int k = 0; k < K; k++) {
    //checks if it reached the end before assigning all values
    if(!getline(iss, w, ';')) {
      printf("Cannot read alpha values from file %s!\n", filename.c_str());
      return 1;
    }
  ...
  //skipping the 'beta=' part on string
  getline(file, line, '=');
  //getting beta
  getline(file, w, '\n');
...
void model::compute_new_alpha_beta() {
  double denominator = 0;
  double * numerator = new double[K];
  double * nd_alpha = new double[K];
  double sum_alpha = 0;
  double sum_nd_alpha = 0;
  ...
  for (int m = 0; m < M; m++) {
    ...
    for (int k = 0; k < K; k++){
      // do all sums needed for the calculations of the denominator
       sum_alpha += alpha[k];
       sum_nd_alpha += nd[m][k] + alpha[k];

       //sums for numerator
       if (alpha[k] != 0)
          numerator[k] += gsl_sf_psi( nd[m][k] + alpha[k]) - gsl_sf_psi( alpha[k]);
    }

    denominator += gsl_sf_psi(sum_nd_alpha) - gsl_sf_psi(sum_alpha);
  }
  ...
  // update alpha values
  for (int k = 0; k < K; k++) {
    alpha[k] *= ( numerator[k] / denominator );
    cout << alpha[k] << " ";
    if (alpha[k] == 0)
      count++;
  }
  cout << endl;
  cout << "# of alphas == 0: " << count << endl;


  //   UPDATE BETA
  denominator = 0.0;
  double numerator2 = 0.0;
  double aux_nw_beta_den;

  for (int k = 0; k < K; k++) {
    aux_nw_beta_den = 0.0;

    for (int w = 0; w < V; w++) {
      numerator2 += gsl_sf_psi(nw[w][k] + beta) - gsl_sf_psi(beta);
      aux_nw_beta_den += (nw[w][k] + beta);
    }

    denominator += gsl_sf_psi(aux_nw_beta_den) - gsl_sf_psi(V * beta);
  }

  beta = ( beta * numerator2 ) / (V * denominator);
...
int model::sampling(int m, int n) {
  ...
  double Vbeta = V * beta;
  double Kalpha = getalphas();

  // do multinomial sampling via cumulative method
  for (int k = 0; k < K; k++) {
    p[k] = (nw[w][k] + beta) / (nwsum[k] + Vbeta) *
        (nd[m][k] + alpha[k]) / (ndsum[m] + Kalpha);
  }
...
void model::compute_theta() {
  ...
  for (int m = 0; m < M; m++) {
    for (int k = 0; k < K; k++) {
      theta[m][k] = (nd[m][k] + alpha[k]) / (ndsum[m] + Kalpha);
    }
  }
}
...
int model::inf_sampling(int m, int n) {
  ...
  // sum of alphas for all K
  double Kalpha = getalphas();

  // do multinomial sampling via cumulative method
  for (int k = 0; k < K; k++) {
    p[k] = (nw[w][k] + newnw[_w][k] + beta) / (nwsum[k] + newnwsum[k] + Vbeta) *
        (newnd[m][k] + alpha[k]) / (newndsum[m] + Kalpha);
  }
  // cumulate multinomial parameters
  for (int k = 1; k < K; k++) {
  p[k] += p[k - 1];
...
void model::compute_newtheta() {
  ...
  for (int m = 0; m < newM; m++) {
    for (int k = 0; k < K; k++) {
      newtheta[m][k] = (newnd[m][k] + alpha[k]) / (newndsum[m] + alphas);
    }
  }
}
